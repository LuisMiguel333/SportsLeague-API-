...
public class CardRepository : GenericRepository<Card>, ICardRepository
{
    ...
    public async Task<IEnumerable<Card>> GetByMatchAsync(int matchId)
        => await _dbSet.Where(c => c.MatchId == matchId).OrderBy(c => c.Minute).ToListAsync();

    public async Task<IEnumerable<Card>> GetByMatchWithDetailsAsync(int matchId)
        => await _dbSet.Where(c => c.MatchId == matchId)
            .Include(c => c.Player).OrderBy(c => c.Minute).ToListAsync();
}